...
def multis_transform_fromOutput_toDictionary_AUTOMATED(outputtedVector, listTrofferOrigins):
    
    masterDictMTXs = {}
    listTrofferDictNames = []
    
    trMaxRange = len(listTrofferOrigins) + 1  
    for tr in range(1, trMaxRange):
        item = "dictMT" + str(tr)
        listTrofferDictNames.append(item)
    
    for item in listTrofferDictNames:
        masterDictMTXs[item] = {}


    names = ["b", "l", "r", "u"]
    allMultis = 53 * len(listTrofferOrigins) # 424 or other
    idMaxRange = allMultis + 1 # 425 or other
    for id in range(1,idMaxRange):
        q, r = divmod(id, 53)
        if (r == 0):
            q -= 1
            r = 53
        ...
        trofferNameKey = "dictMT" + str(q+1)
        
        if (r < 5):
            keyMultiBat = trofferNameKey + "_" + names[r-1]
            masterDictMTXs[trofferNameKey][keyMultiBat] = outputtedVector[id-1] # bat multis for troffer
            #print(f'Light {id} belongs to troffer {q+1} and is its {r} element')
        if (r > 4):
            newID = r - 4
            ...
            x = quo + 1 # row
            y = rem # column
            keyMultiPixel = trofferNameKey + "_" + str(x) + str(y)
            masterDictMTXs[trofferNameKey][keyMultiPixel] = outputtedVector[id-1] # pixel multis for troffer
            #print(f'Light {id} belongs to troffer {q+1} in row {x} and column {y}')

    return masterDictMTXs
...
def solver_BVLS(contrMatrix, dictSPcoords, swapWith, dictActTLV, ecoMode, listOccupantCoords, thresholdsR1R2, dimmerValue, listDistAngleMultis, listTrofferOrigins):
    ...
    res = lsq_linear(contrMatrix, targetVector, bounds=(lb, ub), method='bvls', verbose=0)
    scalars = res.x
    ...
    dictMxBVLS = multis_transform_fromOutput_toDictionary_AUTOMATED(scalars,listTrofferOrigins) # dictM1, dictM2, dictM3, dictM4, dictM5, dictM6, dictM7, dictM8
...
arrayHumanAwareTLVs, listColorVector = humanAware_TLVs(dictSPcoords, tlvOG, tlvES, listOccupantCoords, thresholdsR1R2, dimmerValue)
...
# -----------------------------------------------------------------------------------------
# -----------------------------------------------------------------------------------------
# THE SUCCEEDING PART IS FOR BIDIRECTIONAL COMMUNICATION WITH UNITY - YOU DO NOT NEED THIS
# -----------------------------------------------------------------------------------------
# -----------------------------------------------------------------------------------------
# -----------------------------------------------------------------------------------------
# -----------------------------------------------------------------------------------------
...
while True:
    ...
    msgReceived = sock.recv(1024).decode("UTF-8") # Receiving Byte Data from C# and Converting it to String
    print("------------------------")
    print("------------------------")
    ...
    if (msgReceived != "null"):
        ...
        if(counter == 0 and listMessages[0] == "TrofferOrigins"):
            activity = "null"
            print(f'OG String with Troffer Origins is {listMessages[1]}')
            listStrTrofferOrigins = listMessages[1].split("_")
            listTrofferOrigins = []
            for st in listStrTrofferOrigins:
                ...
                listTrofferOrigins.append(arrayWithVals)
            print(f'Troffer Origins are {listTrofferOrigins}.')
                
        
        if(listMessages[0][0:4] == "swap"):
            act,mode,r1r2,oPos,gVecs,dimVal,glMode = 0,1,2,3,4,5,6
            ...
            if (len(listMessages) > 2):
                ...
                print(f'Threshold1 is {thresholdsR1R2[0]} and Threshold2 is {thresholdsR1R2[1]}')
            if (len(listMessages) > 3):
                print(f'OG String is {listMessages[oPos]}.')
                listStrOccLoc = listMessages[oPos].replace("(","").replace(" ", "").replace(")", " ").rstrip().split(" ")
                listOccupantCoords = []
            ...
            if (len(listMessages) > 4):
                ...
                listStrGazeVectors = listMessages[gVecs].replace(" ", "").replace(")",") ").rstrip().split(" ")
                listGazeVectors = []
                for st in listStrGazeVectors:
                    ...
                    tupleWithVals = tuple(floatValues)
                    arrayWithVals = np.array(floatValues)
                    listGazeVectors.append(arrayWithVals) # list with np arrays
            ...
            if (len(listMessages) > 6):
                ...
                if (glareMode == "False"):
                    ...
                    listAngleMultis, listDistAngleMultis, dictDistAngleMultis = angleAndDistance_gaze_math(listGazeVectors, dictLVs53, th1, th2, gthMax, gthMin, listArrayOccupantCoords, listTrofferOrigins, x1, x2, gmax)
        ...
        if(
           ...
           (activity == "swap ies profiles to Blackboard BVLS")):
            ...
            multisSculpted, energySavings = solver_BVLS_EnergySavings(counter, swapWith, mtx, dictSPcoords, dictActLV, arrayHumanAwareTLVs, ecoMode, listOccupantCoords, thresholdsR1R2, exportAllTrofferMutisPath, listDistAngleMultis)

            counter += 1
            print(f"Counter value is: {counter}")
            messageUnity = "BLEND" + "$" + swapWith + "$" + str("{:.2f}".format(energySavings))
            canSendToUnityBool = True
